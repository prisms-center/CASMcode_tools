...
def search(args):
    ...
    - optionally constructs supercells of the child in a volume range
    ...
    - performs a mapping with max_vol, k_best, min_cost, max_cost, cost type
      ...
      - if n vacancies are allowed, it is lcm(child's # atoms + n, parent's # sites) / (parent's # sites)
    - returns mapping results and the parent and child structures which were used
    - note: vacancy fraction needs to be handled somehow, in case the user passes in weird numbers
    ...
    - the child's number of atoms must be an integer multiple of the parent's number of sites
      ...
      - if vacancies are allowed, the following must be true for some n in [0, # Va allowed]
    ...
    if args.max_vol is None:
        ...
        if parent_natoms > child_natoms:
            print(
                f"incompatible structures: parent has more atoms ({parent_natoms}) than child ({child_natoms})\n"
                "try making supercells of the child or primifying the parent"
            )
            return
        if math.lcm(parent_natoms, child_natoms) != child_natoms:
            print(
                f"incompatible structures: child natoms ({child_natoms}) is not an integer multiple "
                f"of parent natoms ({parent_natoms})\n"
